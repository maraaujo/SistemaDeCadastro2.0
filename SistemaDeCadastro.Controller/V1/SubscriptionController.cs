...
namespace SistemaDeCadastro.Controller.V1
{
    ...
    public class SubscriptionController : ControllerBase
    {
        ...
        [HttpPost("SimulatePaymentApproved/{subscriptionId}")]
        public async Task<IActionResult> SimulatePaymentApproved(long subscriptionId)
        {
            var ret = await _app.SimulatePaymentApproved(subscriptionId);
            return Ok(ret);
        }

        [HttpPost("SimulatePaymentRefused/{subscriptionId}")]
        public async Task<IActionResult> SimulatePaymentRefused(long subscriptionId)
        {
            var ret = await _app.SimulatePaymentRefused(subscriptionId);
            return Ok(ret);
        }

        [HttpGet("GetActiveSubscriptionByInstitution/{institutionId}")]
        public async Task<IActionResult> GetActiveSubscriptionByInstitution(long institutionId)
        {
            var ret = await _app.GetActiveSubscriptionByInstitution(institutionId);
            return Ok(ret);
        }
    }
}
